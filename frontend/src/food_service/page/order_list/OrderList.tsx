...
const OrderList: React.FC<OrderListProps> = ({ selectMenu, setSelectMenu }) => {
  ...
  const subtotal = selectMenu.reduce((sum, item) => sum + item.Amount * item.Quantity, 0).toFixed(2);

  useEffect(() => {
    if (orderDetail && orderDetail.length > 0) {
      setSelectMenu(orderDetail);
    }
  }, [orderDetail, setSelectMenu]);
      ...
      <section className="summary">
        ...
        <div style={{
          ...
          <div style={{
            ...
            <h1 style={{ margin: 0 }}>฿ {subtotal}</h1>
          </div>
          <Link to="/login/checkout" state={{orderDetail: selectMenu, subtotal: subtotal}}>
